        ...
        for group in miss_point:
            ...
            if miss_len <= threshold_len:
                ...
                for i in group:
                    ...
                    if values:
                        imputed_array[i] = np.mean(values)
                    else:
                        imputed_array[i] = 0
...
def process_all_columns(ID, mode="missing", freq_str="5min", extend_str="0min"):
    ...
    for col in target_cols:
        ...
        if mode == "anomaly":
            ...
            imputed, flag = MissingImpute(df, col)
            output[col] = imputed
            output[col + "_flag"] = flag
    ...
    try:
        plot_recent_day(df_forplot, target_cols, mode, ID,
                        anomalies_dict if mode == "anomaly" else None,
