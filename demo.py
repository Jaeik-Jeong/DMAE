...
def AnomalyDetect(df, colname, z_threshold=None):
    try:
        ...
        anomalies = series[anomaly_mask]  # 이상값만 포함된 시리즈
        ...
        traceback.print_exc()
        return df[colname], pd.Series(dtype='float64')
            ...
            if mode == "anomaly" and anomalies_dict is not None:
                ...
                if anomalies is not None and not anomalies.empty:
                    ...
                    if not recent_anomalies.empty:
                        ax.scatter(recent_anomalies.index, recent_anomalies.values, color='red', label='Anomaly', zorder=5)

            ax.set_ylabel(col)
            ax.legend()

        fig.suptitle(f"{mode.capitalize()} Detection - Last 1 Day")
...
def process_all_columns(ID, mode="missing"):
    ...
    for col in target_cols:
        ...
        if mode == "anomaly":
            cleaned, anomalies = AnomalyDetect(df, col)
            output[col] = cleaned
    ...
    output.to_csv(ID + suffix, index_label='date_time')
    ...
    if mode == "anomaly":
        ...
        df_forplot = output.copy()
    
    try:
        plot_recent_day(df_forplot, target_cols, mode, ID, anomalies_dict if mode == "anomaly" else None)
    except Exception as e:
